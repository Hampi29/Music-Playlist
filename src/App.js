...
class App extends Component {
  state = {songsList: initialTracksList, searchInp: ''}

  deleteSong = id => {
    const {songsList} = this.state
    const filtered = songsList.filter(each => each.id !== id)
  ...
  render() {
    const {songsList, searchInp} = this.state
        ...
        {isEmpty ? (
          ...
          <ul className="songslist">
            {filteredList.map(each => (
              <SongCard key={each.id} songItem={each} deleteSong={this.deleteSong} />
            ))}
          </ul>
        )}
      </div>
